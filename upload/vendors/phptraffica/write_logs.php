...
if (!function_exists('log_phpTA')) {
	function log_phpTA($params) {
		...
		// Connect to db
		/*if (PHP_VERSION > 4.2) {
			null = mysql_connect("$server","$user","$password",true) or die("<br>Can not connect to database in count.php: ".mysql_error());
		} else {
			null = mysql_connect("$server","$user","$password") or die("<br>Can not connect to database in count.php: ".mysql_error());
		}
		$phpTA_db = mysql_select_db("$base",null) or die("<br>Can not select base in count.php[47] ".mysql_error());*/
		// Getting the sites array
		$phpTA_sites = phpTrafficA_get_sites(null, $config_table);
		// Getting the cookie
		$phpTrafficA = "";
		if (isset($_COOKIE['phpTrafficA'])) $phpTrafficA = $_COOKIE['phpTrafficA'];
		// Making sure that the $sid matches something in the sites list
		if (!array_key_exists ($sid, $phpTA_sites)) return;
		...
		if (phpTrafficA_bannedIP(null, $config_table, $phpTA_ip)) {
			//mysql_close (null);
			return;
		}
		...
		$tcout = phpTrafficA_logit(null,$config_table,$phpTA_table, $phpTA_domain, $phpTA_To, $phpTA_ip, @$_SERVER["HTTP_USER_AGENT"], $phpTA_servertime, @$_SERVER["HTTP_REFERER"], @$sites[$sid]['crawler'], $phpTA_tmpdir, $ip2c,$phpTA_resolution);
		if ($phpTA_sites[$sid]['counter']) echo $tcout;
		// Close database
		//mysql_close();
	}
}
