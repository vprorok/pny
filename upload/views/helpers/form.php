 ...
 */
	var $__options = array(
		'day' => array(), 'second' => array(), 'minute' => array(), 'hour' => array(),
 ...
 * @return string
 */
	function second($fieldName, $selected = null, $attributes = array(), $showEmpty = true) {
		if ((empty($selected) || $selected === true) && $value = $this->value($fieldName)) {
			if (is_array($value)) {
				extract($value);
				$selected = $sec;
		...
		if (strlen($selected) > 2) {
			$selected = date('s', strtotime($selected));
		} elseif ($selected === false) {
			$selected = null;
		}
		$secondOptions = array();
		...
		if (isset($attributes['interval'])) {
			$secondOptions['interval'] = $attributes['interval'];
			unset($attributes['interval']);
		}
		return $this->select(
			$fieldName . ".sec", $this->__generateOptions('second', $secondOptions),
			$selected, $attributes, $showEmpty
		);
	}
/**
 ...
 */
	function dateTime($fieldName, $dateFormat = 'DMY', $timeFormat = '12', $selected = null, $attributes = array(), $showEmpty = true) {
		...
		$elements = array('Day','Month','Year','Hour','Minute','Second','Meridian');
		$defaults = array(
		...
		switch($timeFormat) {
            case '24+second':
                ...
                $this->second($fieldName, $sec, $selectSecondAttr, $showEmpty);
            break;
			case '24':
				$opt .= $this->hour($fieldName, true, $hour, $selectHourAttr, $showEmpty) . ':' .
				$this->minute($fieldName, $min, $selectMinuteAttr, $showEmpty);
